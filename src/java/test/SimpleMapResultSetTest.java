...
import util.MapResultSet;
import util.di.annotation.Column;
import util.di.annotation.Nested;

/**
 * Test class đơn giản cho MapResultSet
 */
public class SimpleMapResultSetTest {
    ...
    public static class TestOrder {
        ...
        @Nested(prefix = "customer")
        private TestCustomer customer;
        
        @Nested(prefix = "address")
        ...
        public TestCustomer getCustomer() { return customer; }
        public void setCustomer(TestCustomer customer) { this.customer = customer; }
    ...
    public static void main(String[] args) {
        System.out.println("=== Test MapResultSet ===");
        
        // Test 1: Basic mapping
        testBasicMapping();
        
        // Test 2: Nested object mapping
        testNestedMapping();
    ...
    private static void testBasicMapping() {
        ...
        try {
            ...
            rs.addRow("id", 1);
            rs.addRow("name", "John Doe");
            rs.addRow("email", "john@example.com");
            rs.addRow("birthDate", Date.valueOf(LocalDate.of(1990, 5, 15)));
            rs.addRow("createdAt", Timestamp.valueOf(LocalDateTime.of(2023, 1, 1, 10, 30)));
            rs.addRow("loginTime", Time.valueOf(LocalTime.of(9, 0)));
            rs.addRow("salary", new BigDecimal("50000.50"));
            rs.addRow("avatar", "test".getBytes());
            rs.addRow("status", "ACTIVE");
            
            TestUser user = MapResultSet.mapResultSet(rs, TestUser.class);
            
            System.out.println("✓ Basic mapping successful:");
            ...
            System.out.println("  Avatar: " + Arrays.toString(user.getAvatar()));
            System.out.println("  Status: " + user.getStatus());
            
        } catch (Exception e) {
            System.err.println("✗ Basic mapping failed: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private static void testNestedMapping() {
        ...
        try {
            // Tạo mock ResultSet với nested data
            MockResultSet rs = new MockResultSet();
            rs.addRow("orderId", 100);
            rs.addRow("orderNumber", "ORD-001");
            rs.addRow("customer_name", "Jane Smith");
            rs.addRow("customer_email", "jane@example.com");
            rs.addRow("customer_phone", "123-456-7890");
            rs.addRow("address_street", "123 Main St");
            rs.addRow("address_city", "New York");
            rs.addRow("address_country", "USA");
            
            TestOrder order = MapResultSet.mapResultSet(rs, TestOrder.class);
            
            System.out.println("✓ Nested mapping successful:");
            ...
            if (order.getAddress() != null) {
                System.out.println("  Address Street: " + order.getAddress().getStreet());
                System.out.println("  Address City: " + order.getAddress().getCity());
                System.out.println("  Address Country: " + order.getAddress().getCountry());
            }
            
        } catch (Exception e) {
            System.err.println("✗ Nested mapping failed: " + e.getMessage());
            e.printStackTrace();
        }
    }

    // Mock ResultSet class đơn giản để test
    private static class MockResultSet implements ResultSet {
        ...
        // Các method khác của ResultSet interface (dummy implementation)
        @Override public boolean next() throws SQLException { return currentRow++ == 0; }
        ...
        @Override public Date getDate(int columnIndex, java.util.Calendar cal) throws SQLException { return null; }
        @Override public Date getDate(String columnLabel, java.util.Calendar cal) throws SQLException { return null; }
        @Override public Time getTime(int columnIndex, java.util.Calendar cal) throws SQLException { return null; }
        @Override public Time getTime(String columnLabel, java.util.Calendar cal) throws SQLException { return null; }
        @Override public Timestamp getTimestamp(int columnIndex, java.util.Calendar cal) throws SQLException { return null; }
        @Override public Timestamp getTimestamp(String columnLabel, java.util.Calendar cal) throws SQLException { return null; }
        ...
        @Override public void updateNCharacterStream(String columnLabel, java.io.Reader x, long length) throws SQLException {}
        ...
        @Override public void updateNCharacterStream(String columnLabel, java.io.Reader x) throws SQLException {}
