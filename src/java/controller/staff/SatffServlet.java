...
public class SatffServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // Lấy tất cả hợp đồng có trạng thái PENDING hoặc đã assign staff
            List<ContractDTO> contracts = contractService.getAllContracts(); 

            request.setAttribute("contracts", contracts);

            // Forward sang staff.jsp
            request.getRequestDispatcher("/web/staff/staff.jsp").forward(request, response);
